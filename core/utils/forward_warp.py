...
class ForwardWarp(nn.Module):
	...
	def sample_one(self, img, shiftx, shifty, weight):
		...
		flat_basex = torch.arange(0, H).view(-1, 1)[None, None].cuda().long().repeat(N, C, 1, W).view(-1)
		flat_basey = torch.arange(0, W).view(1, -1)[None, None].cuda().long().repeat(N, C, H, 1).view(-1)
		flat_weight = weight.view(-1)
		flat_img = img.contiguous().view(-1)

		# The corresponding positions in I1
		idxn = torch.arange(0, N).view(N, 1, 1, 1).long().cuda().repeat(1, C, H, W).view(-1)
		idxc = torch.arange(0, C).view(1, C, 1, 1).long().cuda().repeat(N, 1, H, W).view(-1)
